...
const withIOSCrashFix = (config) => {
  ...
  config = withInfoPlist(config, (config) => {
    ...
    const expectedVersion = config.expo?.ios?.version || config.expo?.version || config.version || '1.1.2';
    ...
    const expectedBuildNumber = config.expo?.ios?.buildNumber || config.ios?.buildNumber || config.buildNumber || '37';
