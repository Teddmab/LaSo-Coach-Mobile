...
const NutritionScreen: React.FC<NutritionScreenProps> = ({ user, onLogout, onTabPress, activeTab, onSubscriptionRenew, onFAQPress }) => {
  ...
  const pastIncompleteMeals = useMemo((): Array<{ meal: Meal; date: Date; planDay: number }> => {
    ...
    // ✅ ÉVITER LES RECALCULS INUTILES : Ne pas logger si les données ne sont pas disponibles
    // Cela évite les logs répétitifs pendant le chargement initial
    if (!currentPlan || !completionDataToUse) {
      if (__DEV__) {
        console.log('⚠️ [PAST MEALS] useMemo - Données manquantes:', {
          ...
          hasCompletionDataToUse: !!completionDataToUse,
        });
      }
      return [];
    }
    
    // ✅ TOUJOURS logger quand on recalcule avec des données disponibles
    console.log('🔄 [PAST MEALS] useMemo recalculé - Calcul des plats passés non complétés:', {
      hasCurrentPlan: !!currentPlan,
      currentPlanId: currentPlan?.id,
      hasFreshData: !!freshCompletionData,
      hasCompletionStatus: !!completionStatus,
      usingData: freshCompletionData ? 'freshCompletionData' : 'completionStatus',
      ...
      progressRemainingMeals: completionDataToUse?.progress?.remainingMeals,
      progressCompletedMeals: completionDataToUse?.progress?.completedMeals,
      progressTotalMeals: completionDataToUse?.progress?.totalMeals,
    });
    
    if (__DEV__) {
      console.log('✅ [PAST MEALS] useMemo - Données disponibles, début du calcul:', {
        hasCurrentPlan: !!currentPlan,
        currentPlanId: currentPlan?.id,
        hasFreshData: !!freshCompletionData,
        hasCompletionStatus: !!completionStatus,
    ...
    if (__DEV__) {
      console.log('📅 [PAST MEALS] Calcul des dates:', {
        todayDate: todayDate.toDateString(),
      });
    }

    // ✅ LOGIQUE UNIVERSELLE : Déterminer la date de début du plan (date d'inscription)
    // Pour un nouveau compte : utiliser aujourd'hui (date d'inscription)
    // Pour un ancien compte : utiliser la date d'inscription réelle
    
    // Détecter si c'est un nouveau compte (pas de complétions)
    ...
    if (isNewAccount) {
      // ✅ NOUVEAU COMPTE : Date d'inscription = aujourd'hui
      planStartDate = todayDate;
      dateSource = 'new account (today)';
      
      if (__DEV__) {
        console.log('🆕 [PAST MEALS] Nouveau compte détecté - Date d\'inscription = aujourd\'hui');
      }
      
      // ✅ IMPORTANT: Si le plan commence aujourd'hui, il n'y a pas de jours passés
      return [];
    } else {
      // ✅ ANCIEN COMPTE : Utiliser la date d'inscription réelle
      // Priorité : profile.createdAt > subscription.startDate > plan.startDate > today
      if (profileData?.createdAt) {
        ...
        planStartDate = todayDate;
        dateSource = 'today (fallback)';
      }
      
      // ✅ IMPORTANT: Si le plan commence aujourd'hui ou dans le futur, il n'y a pas de jours passés
      if (planStartDate >= todayDate) {
        if (__DEV__) {
          console.log('📅 [PAST MEALS] Plan commence aujourd\'hui ou dans le futur - Aucun plat passé à compléter', {
    ...
    if (__DEV__) {
      console.log('📅 [PAST MEALS] Date de début du plan déterminée:', {
        planStartDate: planStartDate.toDateString(),
        dateSource,
        isNewAccount,
        todayDate: todayDate.toDateString(),
    ...
    if (__DEV__) {
      console.log('📅 [PAST MEALS] Dates calculées:', {
        planStartDate: planStartDate.toDateString(),
        dateSource,
        isNewAccount,
        yesterday: yesterday.toDateString(),
        startDate: startDate.toDateString(),
        willLoop: startDate <= yesterday,
      });
    }

    // ✅ NOUVELLE LOGIQUE: Calculer par jour le nombre de plats non complétés
    ...
    for (let date = new Date(startDate); date <= yesterday; date.setDate(date.getDate() + 1)) {
      ...
      if (dayMenu && dayMenu.meals) {
        // ✅ Compter le nombre total de repas pour ce jour
        const totalMeals = dayMenu.meals.length;
        
        // ✅ Compter le nombre de repas complétés pour ce jour (via completionsByDay)
        const dayKey = String(planDay);
        const dayCompletions = completionDataToUse?.completionsByDay?.[dayKey] || completionDataToUse?.completionsByDay?.[planDay] || [];
        const completedMeals = Array.isArray(dayCompletions) ? dayCompletions.length : 0;
        
        // ✅ Calculer le nombre de plats non complétés pour ce jour
        const incompleteMeals = totalMeals - completedMeals;
        
        // Ajouter les statistiques du jour
        dayStats.push({
          date: dateCopy,
          planDay,
          totalMeals,
        ...
        // ✅ Log détaillé pour identifier les plats complétés vs non complétés
        if (__DEV__) {
          const completedMealIds = new Set<string>();
          if (Array.isArray(dayCompletions)) {
            dayCompletions.forEach((c: any) => {
              if (c?.mealId) {
                completedMealIds.add(c.mealId);
              }
            });
          }
          
          const allMealIds = dayMenu.meals.map((m: Meal) => m.id);
          const incompleteMealIds = allMealIds.filter((id: string) => !completedMealIds.has(id));
          
          console.log(`📊 [PAST MEALS] Jour ${planDay} (${dateCopy.toDateString()}):`, {
            totalMeals,
            completedMeals,
            incompleteMeals,
            completedMealNames: dayMenu.meals
              .filter((m: Meal) => completedMealIds.has(m.id))
              .map((m: Meal) => `${m.name} (${m.id})`),
            incompleteMealNames: dayMenu.meals
              .filter((m: Meal) => !completedMealIds.has(m.id))
              .map((m: Meal) => `${m.name} (${m.id})`),
            note: `Les plats incomplets seront ajoutés à pastMeals`,
          });
        }
        
        // ✅ Pour chaque repas, vérifier individuellement s'il est complété (MÊME LOGIQUE que CompleteMealsBottomSheet)
        dayMenu.meals.forEach((meal: Meal) => {
          // ✅ Vérifier si ce repas est complété pour ce planDay spécifique
          // IMPORTANT: Utiliser completionDataToUse qui est freshCompletionData || completionStatus
          const isCompleted = isMealCompleted(meal.id, completionDataToUse, planDay);
          
          // ✅ Log détaillé pour déboguer
          if (__DEV__) {
            const dayKey = String(planDay);
            const dayCompletions = completionDataToUse?.completionsByDay?.[dayKey] || completionDataToUse?.completionsByDay?.[planDay] || [];
            const mealIdInCompletions = Array.isArray(dayCompletions) ? dayCompletions.some((c: any) => c.mealId === meal.id) : false;
            
            console.log(`🔍 [PAST MEALS] Vérification plat ${meal.name} (${meal.id}) pour planDay ${planDay}:`, {
              planDay,
              dayKey,
              isCompleted,
          ...
          // ✅ NE PAS ajouter si le plat est complété (comme CompleteMealsBottomSheet)
          // Seuls les plats NON complétés sont ajoutés à pastMeals
          if (!isCompleted) {
            pastMeals.push({
              meal,
              date: dateCopy,
            ...
            if (__DEV__) {
              console.log(`✅ [PAST MEALS] Plat ${meal.name} (${meal.id}) AJOUTÉ à pastMeals car NON complété pour planDay ${planDay}`);
            }
          } else {
            // Log pour vérifier que les plats complétés sont bien exclus
            if (__DEV__) {
              console.log(`🚫 [PAST MEALS] Plat ${meal.name} (${meal.id}) EXCLU de pastMeals car complété pour planDay ${planDay}`);
            }
          }
        });
      }
    ...
    if (__DEV__) {
      console.log('📅 [PAST MEALS] Boucle terminée:', {
        daysProcessed,
    ...
    // ✅ SIMPLIFIÉ: Calculer la somme totale des plats non complétés (comme remainingMeals dans progress)
    // Pour chaque jour passé: incompleteMeals = totalMeals - completedMeals
    // On somme tous les incompleteMeals de tous les jours passés
    const totalIncompleteMeals = dayStats.reduce((sum, day) => sum + day.incompleteMeals, 0);
    
    // ✅ TOUJOURS logger pour voir le calcul (pas seulement en __DEV__)
    console.log('📅 [PAST MEALS] Calcul par jour des plats non complétés (comme remainingMeals):', {
      totalIncompleteMeals,
      pastMealsCount: pastMeals.length,
      ...
      dayStats: dayStats.map(day => ({
        ...
        incompleteMeals: day.incompleteMeals, // totalMeals - completedMeals pour ce jour
      })),
      note: 'totalIncompleteMeals = somme de (totalMeals - completedMeals) pour tous les jours passés'
    });

    // ✅ Stocker le nombre total dans une propriété spéciale pour l'utiliser dans le bouton
    (pastMeals as any).__totalIncompleteMeals = totalIncompleteMeals;

    return pastMeals;
  }, [currentPlan, freshCompletionData, completionStatus, subscriptionData, profileData, isMealCompleted, calculatePlanDayFromDate, findMenuForPlanDay]);

  // ✅ SIMPLIFIÉ: Utiliser directement la somme calculée dans pastIncompleteMeals (comme remainingMeals dans progress)
  // Pour chaque jour passé: incompleteMeals = totalMeals - completedMeals
  // totalPastIncompleteMeals = somme de tous les incompleteMeals de tous les jours passés
  const totalPastIncompleteMeals = useMemo(() => {
    // ✅ Utiliser la propriété __totalIncompleteMeals stockée dans pastIncompleteMeals
    // C'est la somme de (totalMeals - completedMeals) pour tous les jours passés
    // Même logique simple que remainingMeals dans progress
    const count = (pastIncompleteMeals as any).__totalIncompleteMeals ?? pastIncompleteMeals.length;
    
    if (__DEV__) {
      console.log('📊 [PAST MEALS] Nombre total de plats passés non complétés (comme remainingMeals):', {
        totalIncomplete: count,
        calculatedFrom: (pastIncompleteMeals as any).__totalIncompleteMeals ? 'dayStats (somme)' : 'pastIncompleteMeals.length',
        hasFreshData: !!freshCompletionData,
        hasCompletionStatus: !!completionStatus,
        usingData: freshCompletionData ? 'freshCompletionData' : completionStatus ? 'completionStatus' : 'none',
        hasCompletionsByDay: (freshCompletionData || completionStatus)?.completionsByDay ? Object.keys((freshCompletionData || completionStatus).completionsByDay) : [],
        note: 'Calcul simple: somme de (totalMeals - completedMeals) pour tous les jours passés',
      });
    }

    return count;
  }, [pastIncompleteMeals, freshCompletionData, completionStatus]);
  ...
  useFocusEffect(
    useCallback(() => {
      // ✅ ÉVITER LE DOUBLE APPEL : Ne pas appeler fetchAllData si on est déjà en train de charger
      // OU si le premier chargement n'est pas encore terminé
      if (isFetchingAllData || isLoadingDayData || !hasInitialLoadRef.current) {
        if (__DEV__) {
          console.log('⏳ [NutritionScreen] useFocusEffect - Skipping (already loading or initial load not done)', {
            isFetchingAllData,
            isLoadingDayData,
            hasInitialLoad: hasInitialLoadRef.current,
          });
  ...
  // ✅ FIX: Un nouveau compte commence TOUJOURS au day1, peu importe la date d'inscription
  const calculateNutritionPlanDay = (selectedDate: Date | number): number => {
    ...
    // ✅ DÉTECTION D'UN NOUVEAU COMPTE : Vérifier si l'utilisateur a déjà complété des repas
    const hasNoCompletions = !completionStatus || 
                             (completionStatus.progress?.completedMeals === 0 || 
                              !completionStatus.progress?.completedMeals);
    const hasNoCompletionsByDay = !completionStatus?.completionsByDay || 
                                   Object.keys(completionStatus.completionsByDay).length === 0;
    const isNewAccount = hasNoCompletions && hasNoCompletionsByDay;
    
    let referenceDate: Date;
    let dateSource: string;
    
    // ✅ NOUVEAU COMPTE : Toujours utiliser la date actuelle comme référence pour commencer au day1
    if (isNewAccount) {
      referenceDate = new Date();
      referenceDate.setHours(0, 0, 0, 0);
      dateSource = 'new account (today)';
      
      logger.debug('New account detected - Starting at day 1', {
        hasCompletionStatus: !!completionStatus,
        completedMeals: completionStatus?.progress?.completedMeals,
      ...
      // ✅ COMPTE EXISTANT : Utiliser la logique normale avec priorité
  ...
  const fetchCompletionStatus = async (planId: string) => {
    ...
    try {
      ...
      // ✅ Réduire les logs verbeux - seulement logger en mode dev et de manière concise
      if (__DEV__) {
        console.log('📥 [COMPLETION STATUS] Réponse reçue:', {
          ...
          completionsByDayKeys: globalCompletionData?.data?.completionsByDay ? Object.keys(globalCompletionData.data.completionsByDay) : [],
        });
      }
      ...
      // ✅ IMPORTANT: Pour un nouveau compte, initialiser avec des valeurs par défaut
      // Cela permet d'afficher la carte de progression même sans données de complétion
  ...
  const loadDayData = async (planToUse?: NutritionPlan | null) => {
    ...
    try {
      ...
      // ✅ DÉTECTION D'UN NOUVEAU COMPTE : Vérifier si l'utilisateur a déjà complété des repas
      const completionDataToCheck = freshCompletionData || completionStatus;
      const hasNoCompletions = !completionDataToCheck || 
                               (completionDataToCheck.progress?.completedMeals === 0 || 
                                !completionDataToCheck.progress?.completedMeals);
      const hasNoCompletionsByDay = !completionDataToCheck?.completionsByDay || 
                                     Object.keys(completionDataToCheck.completionsByDay).length === 0;
      const isNewAccount = hasNoCompletions && hasNoCompletionsByDay;
      
      // Determine plan start date
      let planStartDate = today;
      
      // ✅ NOUVEAU COMPTE : Toujours utiliser la date actuelle pour commencer au day1
      if (isNewAccount) {
        planStartDate = today;
        if (__DEV__) {
          console.log('🆕 [LOAD DAY DATA] Nouveau compte détecté - Utilisation de la date actuelle pour commencer au day1');
        }
      } else {
        // ✅ COMPTE EXISTANT : Utiliser la logique normale
    ...
    try {
      ...
      if (currentPlan?.id) {
        ...
        try {
          ...
          // ✅ IMPORTANT: Utiliser la même structure que newCompletionStatus pour garantir la cohérence
          ...
          if (__DEV__) {
            ...
            console.log('✅ [MEAL COMPLETE] freshCompletionData mis à jour après complétion (PERSISTANT):', {
              mealId,
              planDay: planDayForRefresh,
              ...
              progressCompletedMeals: newFreshData?.progress?.completedMeals,
              progressTotalMeals: newFreshData?.progress?.totalMeals,
              note: 'Ces données persisteront et pastIncompleteMeals/totalPastIncompleteMeals se recalculeront automatiquement',
            });
          }
    ...
    <>
      ...
      >
        ...
        {(plansResponseStatus === 200 || (currentPlan && hasActiveSubscription) || dayMeals.length > 0) && (
          <View style={styles.mealsContainer}>
            ...
            {totalPastIncompleteMeals > 0 && (
              <TouchableOpacity
                style={styles.pastMealsButton}
                onPress={async () => {
                  // ✅ Rafraîchir le statut de complétion depuis le serveur AVANT d'ouvrir le bottomsheet
                  // (même logique que le bouton "Compléter des repas")
                  if (currentPlan?.id) {
                    try {
                      console.log('🔄 [PAST MEALS] Rafraîchissement du statut depuis le serveur...');
                      const apiResponse = await nutritionAPI.getCompletionStatus(currentPlan.id);
                      
                      // Log détaillé de la réponse du backend
                      console.log('📥 [PAST MEALS] Réponse complète du backend:', JSON.stringify(apiResponse, null, 2));
                      
                      // ✅ IMPORTANT: Extraire depuis .data (même logique que fetchCompletionStatus)
                      const globalCompletionData = apiResponse?.data || apiResponse;
                      
                      // Log de la structure des données après parsing
                      console.log('📊 [PAST MEALS] Structure des données de complétion:', {
                      ...
                      if (__DEV__) {
                        console.log('📊 [PAST MEALS] Données avant mise à jour de freshCompletionData:', {
                          hasCompletionsByDay: !!newFreshData?.completionsByDay,
                          completionsByDayKeys: newFreshData?.completionsByDay ? Object.keys(newFreshData.completionsByDay) : [],
                          totalCompletions: newFreshData?.completionsByDay ? Object.values(newFreshData.completionsByDay).reduce((total: number, dayCompletions: any) => {
                            return total + (Array.isArray(dayCompletions) ? dayCompletions.length : 0);
                          }, 0) : 0,
                          hasCurrentPlan: !!currentPlan,
                          currentPlanId: currentPlan?.id,
                      ...
                      // ✅ Mettre à jour completionStatus AVANT freshCompletionData pour garantir la disponibilité
                      setCompletionStatus(prevStatus => ({
                        ...prevStatus,
                        ...newFreshData,
                      ...
                      // ✅ Mettre à jour freshCompletionData APRÈS completionStatus
                      setFreshCompletionData(newFreshData);
                      
                      if (__DEV__) {
                        console.log('✅ [PAST MEALS] États mis à jour:', {
                          hasNewFreshData: !!newFreshData,
                          hasCompletionsByDay: !!newFreshData?.completionsByDay,
                          hasCurrentPlan: !!currentPlan,
                          note: 'Le useMemo devrait se recalculer maintenant avec les nouvelles données',
                        });
                      }
                      
                      console.log('✅ [PAST MEALS] Statut mis à jour, attente du recalcul de pastIncompleteMeals...');
                      console.log('⏳ [PAST MEALS] Le useMemo devrait se recalculer automatiquement et afficher le log "📅 [PAST MEALS] Calcul par jour..."');
                      
                      // ✅ Attendre que React mette à jour les états et recalcule pastIncompleteMeals
                      // Utiliser requestAnimationFrame + setTimeout pour garantir que le render est terminé
                      await new Promise(resolve => {
                        requestAnimationFrame(() => {
                          setTimeout(() => {
                            // Log pour vérifier que pastIncompleteMeals s'est bien recalculé
                            if (__DEV__) {
                              console.log('✅ [PAST MEALS] Vérification après mise à jour:', {
                                hasFreshData: !!newFreshData,
                                hasCompletionsByDay: !!newFreshData?.completionsByDay,
                                completionsByDayKeys: newFreshData?.completionsByDay ? Object.keys(newFreshData.completionsByDay) : [],
                                note: 'pastIncompleteMeals devrait se recalculer automatiquement grâce au useMemo',
                                expectedLog: 'Le log "📅 [PAST MEALS] Calcul par jour..." devrait apparaître ci-dessus',
                              });
                            }
                            console.log('✅ [PAST MEALS] Ouverture du bottomsheet avec données fraîches');
                            setShowPastMealsBottomSheet(true);
                            resolve(undefined);
                          }, 100); // Délai pour garantir le recalcul
                        });
                      });
                    } catch (error) {
                      console.error('❌ [PAST MEALS] Erreur lors du rafraîchissement du statut:', error);
                      // En cas d'erreur, ouvrir quand même le bottomsheet avec les données actuelles
                      setShowPastMealsBottomSheet(true);
                    }
                  } else {
                    // Si pas de plan, ouvrir directement avec les données actuelles
                    setShowPastMealsBottomSheet(true);
                  }
                }}
              >
              ...
              onPress={async () => {
                ...
                if (currentPlan?.id) {
                  try {
                    ...
                    // Pour les autres erreurs, afficher un message et continuer avec les données locales
                    if (errorStatus) {
                      Toast.show({
                        type: 'info',
                        text1: 'Information',
                        text2: 'Impossible de rafraîchir le statut. Utilisation des données locales.',
                        visibilityTime: 2000,
                      });
                    }
      ...
      <PastMealsBottomSheet
        ...
        onMealComplete={async (mealId: string, planDay: number) => {
          // handleMealComplete met déjà à jour freshCompletionData et completionStatus
          await handleMealComplete(mealId, planDay);
        }}
        // ✅ RETIRÉ: onUpdateCompletionData n'est plus nécessaire
        // handleMealComplete met déjà à jour freshCompletionData avec les données du serveur
        // Le useEffect dans PastMealsBottomSheet synchronise localCompletionData avec freshCompletionData automatiquement
        isIOS={isIOS}
      />
